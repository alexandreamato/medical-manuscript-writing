...
| `section_limits` | `{<section id>: {unit, max, min}}`, e.g. a Discussion cap. `unit: "sentences"` counts sentences; `unit: "items"` counts list items (e.g. a key-points box of up to three bullets); works for level-1 and level-2 sections. |
| `references` | `{max, min, soft, soft_note, require_identifier}` (identifier = DOI or PMID). `soft: true` for a threshold that is not an editorial limit (e.g. a fee above it): a WARN under `limits`, never blocking. |
| `figures`, `tables` | `{max, placement}`; tables also `numbering: "arabic" \| "roman"` (Table I, II…). Tables: `inline` or `end` (after references). Figures: `inline` or `legends-at-end` (images shipped as separate numbered files). |
