...
The version is recorded in two places: `CHANGELOG.md` (current: `1.7.0` — 2026-09-23) and (plugin format only) `.claude-plugin/plugin.json`. `dist/build-zips.sh` reads the version from `CHANGELOG.md` and writes it into `plugin.json`, so bump only the changelog, then run `bash dist/build-zips.sh`.
