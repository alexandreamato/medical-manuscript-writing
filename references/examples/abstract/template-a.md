...
```
Background
% [Clinical importance and current evidence gap.]

Methods
% [Design + population + intervention + comparator + primary outcome + duration.]

Results
% [Primary outcome with effect estimate, 95% CI, and (where appropriate) p value. Then key secondary or safety result.]

Conclusions
% [One-sentence take-home matching the design's causal license. Include scope.]

Trial registration
% [Registry and identifier, e.g., ClinicalTrials.gov NCT01234567.]
```

## Reusable sentence skeleton

1. `[Condition] is [importance / burden statement].`
2. `Whether [intervention] reduces [outcome] in [population] is uncertain because [methodological gap of prior evidence].`
3. `In this [design], we randomly assigned [N] participants with [eligibility criterion] to receive [intervention] or [comparator] for [duration]. The primary outcome was [outcome] at [timepoint].`
4. `The primary outcome occurred in [n/N (%)] of the intervention group vs. [n/N (%)] of the comparator group, an absolute difference of [X] percentage points (95% CI, [lower] to [upper]) and a relative risk of [point estimate] (95% CI, [lower] to [upper]; p = [value]).`
...
```
Background
Heart failure with preserved ejection fraction (HFpEF) is a leading cause of hospitalization in older adults. SGLT2 inhibitors reduce hospitalizations in heart failure with reduced ejection fraction, but evidence in HFpEF is limited to single trials with short follow-up.
